...
export default {
  ...
  watch: {
    ...
    internalValue(newValue, oldValue) {
      ...
      this.validateEmit('change', this.getValue())
    },
  ...
  methods: {
    ...
    handleClickOutside(evt) {
      ...
      // istanbul ignore else
      if ((this.$refs.wrapper && !this.$refs.wrapper.contains(evt.target)) && evt.target.id !== 'vue-treeselect-container') {
        console.log('关闭')
        this.blurInput()
        this.closeMenu()
      }
    ...
    closeMenu() {
      ...
      this.validateEmit('blur', this.getValue())
    },
    ...
    openMenu() {
      ...
      this.validateEmit('focus', this.getInstanceId())
    },
    validateEmit (eventName, params) {
      console.log('validateEmit');
      ...
      if (parent) {
        parent.$emit('el.form.' + eventName, params)
      }
    },
    toggleMenu() {
      if (this.menu.isOpen) {
